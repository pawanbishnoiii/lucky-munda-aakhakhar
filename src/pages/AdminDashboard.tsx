...
import { ArrowLeft, Gamepad2, Wallet, Users, Settings, BarChart3, MessageSquare, Bell, Plus, Check, X, Eye, Download, ChevronLeft, ChevronRight, Edit2, Trash2, EyeOff, Search, Filter, RefreshCw, DollarSign, Hash, Clock, UserCheck, AlertTriangle } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const fetchAll = async () => {
    setRefreshing(true);
    const [g, t, tk, s, p, r, b, w] = await Promise.all([
  ...
  const declareResult = async () => {
    ...
    // Auto-payout
    const { data: winBets } = await supabase.from("bets").select("*").eq("game_id", declareGameId).eq("bet_date", selectedCalDate).eq("status", "pending");
    if (winBets && winBets.length > 0) {
      ...
      for (const bet of winBets) {
        if (bet.number === aakharNum) {
          const netWin = bet.amount * payoutMultiplier;
  ...
  const filteredBets = bets.filter(b => {
    ...
    if (betSearch) {
      ...
      if (!gameName.includes(betSearch.toLowerCase()) && !b.user_id.includes(betSearch)) return false;
    }
    return true;
  });
      ...
      <div className="px-4 pt-4">
          ...
          <div className="space-y-3">
            ...
            {pendingTxns > 0 && (
              <div className="bg-accent/10 border border-accent/30 rounded-xl p-3 flex items-center gap-2">
                ...
                <p className="text-accent text-sm font-medium">{pendingTxns} pending transaction(s) need attention!</p>
        ...
        {tab === "games" && (
          <div className="space-y-3">
            ...
            {showAddGame && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-xl p-4 space-y-2 border border-primary/20">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div><label className="text-muted-foreground text-[10px]">Result Time</label><input type="time" value={newGame.result_time} onChange={(e) => setNewGame({ ...newGame, result_time: e.target.value })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                  <div><label className="text-muted-foreground text-[10px]">Close Before (min)</label><input type="number" value={newGame.closure_minutes} onChange={(e) => setNewGame({ ...newGame, closure_minutes: parseInt(e.target.value) || 30 })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div><label className="text-muted-foreground text-[10px]">Payout %</label><input type="number" value={newGame.payout_percentage} onChange={(e) => setNewGame({ ...newGame, payout_percentage: parseFloat(e.target.value) || 93 })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                  <div><label className="text-muted-foreground text-[10px]">Commission %</label><input type="number" value={newGame.commission_percentage} onChange={(e) => setNewGame({ ...newGame, commission_percentage: parseFloat(e.target.value) || 7 })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                </div>
                <div className="flex gap-2">
                  <button onClick={addGame} className="flex-1 gradient-primary text-primary-foreground py-2 rounded-xl font-semibold text-sm">Add Game</button>
                  <button onClick={() => setShowAddGame(false)} className="flex-1 bg-secondary text-secondary-foreground py-2 rounded-xl text-sm">Cancel</button>
                </div>
              </motion.div>
            )}
            ...
            {editingGame && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-xl p-4 space-y-2 border border-primary/30">
                ...
                <input value={editingGame.name} onChange={(e) => setEditingGame({ ...editingGame, name: e.target.value })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" placeholder="English Name" />
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div><label className="text-muted-foreground text-[10px]">Result Time</label><input type="time" value={editingGame.result_time} onChange={(e) => setEditingGame({ ...editingGame, result_time: e.target.value })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                  <div><label className="text-muted-foreground text-[10px]">Close Before (min)</label><input type="number" value={editingGame.closure_minutes} onChange={(e) => setEditingGame({ ...editingGame, closure_minutes: parseInt(e.target.value) || 30 })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div><label className="text-muted-foreground text-[10px]">Payout %</label><input type="number" value={editingGame.payout_percentage} onChange={(e) => setEditingGame({ ...editingGame, payout_percentage: parseFloat(e.target.value) || 93 })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                  <div><label className="text-muted-foreground text-[10px]">Commission %</label><input type="number" value={editingGame.commission_percentage} onChange={(e) => setEditingGame({ ...editingGame, commission_percentage: parseFloat(e.target.value) || 7 })} className="w-full bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none" /></div>
                </div>
                <div className="flex items-center gap-2">
                  <label className="text-muted-foreground text-xs">Active:</label>
                  <button onClick={() => setEditingGame({ ...editingGame, is_active: !editingGame.is_active })} className={`px-3 py-1 rounded-lg text-xs font-bold ${editingGame.is_active ? "bg-game-green/20 text-game-green" : "bg-destructive/20 text-destructive"}`}>{editingGame.is_active ? "ON" : "OFF"}</button>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setEditingGame(null)} className="flex-1 bg-secondary text-secondary-foreground py-2 rounded-xl text-sm">Cancel</button>
                </div>
              </motion.div>
            )}

            {games.map((g) => (
              <div key={g.id} className={`bg-card rounded-xl p-4 border transition-all ${g.is_active ? "border-border/50" : "border-destructive/30 opacity-60"}`}>
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <p className="text-foreground font-bold text-sm">{g.name_hindi || g.name}</p>
                    <p className="text-muted-foreground text-xs">{g.name}</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => setEditingGame({ ...g })} className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center"><Edit2 className="w-3.5 h-3.5 text-primary" /></button>
                    <button onClick={() => toggleGame(g.id, g.is_active)} className="w-7 h-7 rounded-lg bg-secondary flex items-center justify-center">
                      {g.is_active ? <Eye className="w-3.5 h-3.5 text-game-green" /> : <EyeOff className="w-3.5 h-3.5 text-destructive" />}
                    </button>
                  </div>
                </div>
                <div className="grid grid-cols-4 gap-2">
                  <div className="bg-secondary rounded-lg p-2 text-center"><p className="text-[9px] text-muted-foreground">Time</p><p className="text-foreground font-bold text-xs">{g.result_time}</p></div>
                  <div className="bg-secondary rounded-lg p-2 text-center"><p className="text-[9px] text-muted-foreground">Close</p><p className="text-foreground font-bold text-xs">{g.closure_minutes}m</p></div>
                  <div className="bg-secondary rounded-lg p-2 text-center"><p className="text-[9px] text-muted-foreground">Payout</p><p className="text-game-green font-bold text-xs">{g.payout_percentage}%</p></div>
                  <div className="bg-secondary rounded-lg p-2 text-center"><p className="text-[9px] text-muted-foreground">Com.</p><p className="text-accent font-bold text-xs">{g.commission_percentage}%</p></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-card rounded-xl p-3 border border-border/50">
              ...
              <div className="grid grid-cols-7 gap-1 mb-1">
                {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => <div key={d} className="text-center text-muted-foreground text-[10px] font-medium">{d}</div>)}
              </div>
                ...
                {calDays.map(day => {
                  ...
                  return (
                    <button key={day} onClick={() => setSelectedCalDate(dateStr)}
                      className={`w-full aspect-square rounded-lg text-xs font-medium flex items-center justify-center transition-all ${isSelected ? "gradient-primary text-primary-foreground" : hasResults ? "bg-game-green/15 text-game-green font-bold" : "bg-secondary text-foreground hover:bg-secondary/80"}`}>
                      {day}
                    </button>
                  );
                })}
            ...
            {/* Results for date */}
            <div className="bg-card rounded-xl p-3 border border-border/50">
              <h4 className="font-display font-bold text-foreground text-sm mb-2">📅 {selectedCalDate}</h4>
                  ...
                  {getResultsForDate(selectedCalDate).map(r => (
                    <div key={r.id} className="flex items-center justify-between bg-secondary rounded-lg px-3 py-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => editResult(r)} className="w-6 h-6 rounded bg-primary/10 flex items-center justify-center"><Edit2 className="w-3 h-3 text-primary" /></button>
                        <button onClick={() => deleteResult(r.id)} className="w-6 h-6 rounded bg-destructive/10 flex items-center justify-center"><Trash2 className="w-3 h-3 text-destructive" /></button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-card rounded-xl p-4 space-y-2 border border-primary/20">
              <h4 className="font-display font-bold text-foreground text-sm">🎯 Declare Result ({selectedCalDate})</h4>
              ...
              <button onClick={declareResult} className="w-full gradient-primary text-primary-foreground py-2.5 rounded-xl font-semibold text-sm">Declare & Auto-Payout</button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2 items-center">
              <div className="flex-1 bg-card rounded-xl px-3 py-2 flex items-center gap-2 border border-border/50">
                ...
                <input placeholder="Search game/user..." value={betSearch} onChange={(e) => setBetSearch(e.target.value)} className="bg-transparent text-foreground text-xs w-full outline-none placeholder:text-muted-foreground" />
              </div>
            </div>
            ...
            {filteredBets.slice(0, 100).map((b) => (
              <div key={b.id} className="bg-card rounded-xl p-3 border border-border/50">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    <p className="text-foreground font-bold text-sm">{(b as any).games?.name_hindi || (b as any).games?.name}</p>
                    <p className="text-muted-foreground text-[10px]">User: {b.user_id.slice(0, 8)}...</p>
                  </div>
                ...
                <p className="text-muted-foreground text-[10px] mt-1">⏰ {new Date(b.created_at).toLocaleString("hi-IN")}</p>
              </div>
            ))}
          </div>
        )}
              ...
              <div key={tx.id} className="bg-card rounded-xl p-3 space-y-2 border border-border/50">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-muted-foreground text-[10px]">User: {tx.user_id.slice(0, 8)}... · {tx.payment_method || "-"}</p>
        ...
        {tab === "users" && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 bg-card rounded-xl px-3 py-2 flex items-center gap-2 border border-border/50">
                <Search className="w-3.5 h-3.5 text-muted-foreground" />
                <input placeholder="Search name/phone..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} className="bg-transparent text-foreground text-xs w-full outline-none placeholder:text-muted-foreground" />
              </div>
            </div>

            {/* Selected User Detail */}
            {selectedUser && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-xl p-4 border border-primary/30 space-y-3">
                ...
                <div>
                  ...
                  {userBets.slice(0, 10).map(b => (
                    <div key={b.id} className="flex justify-between items-center bg-secondary rounded-lg px-3 py-1.5 mb-1 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${b.status === "won" ? "bg-game-green/10 text-game-green" : b.status === "lost" ? "bg-destructive/10 text-destructive" : "bg-accent/10 text-accent"}`}>{b.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  {userTxns.slice(0, 10).map(t => (
                    <div key={t.id} className="flex justify-between items-center bg-secondary rounded-lg px-3 py-1.5 mb-1 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${t.status === "pending" ? "bg-accent/10 text-accent" : "bg-game-green/10 text-game-green"}`}>{t.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {filteredUsers.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => loadUserDetails(p)} className="w-full bg-card rounded-xl p-3 border border-border/50 flex items-center justify-between text-left hover:border-primary/30 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 gradient-primary rounded-xl flex items-center justify-center"><UserCheck className="w-4 h-4 text-primary-foreground" /></div>
                    <div>
          ...
          <div className="space-y-3">
            ...
            {tickets.map((t) => (
              <div key={t.id} className="bg-card rounded-xl p-3 space-y-2 border border-border/50">
                <div className="flex justify-between items-start">
                  <div><p className="text-foreground font-medium text-sm">{t.subject}</p><p className="text-muted-foreground text-xs">{t.message}</p><p className="text-muted-foreground text-[10px]">{new Date(t.created_at).toLocaleString("hi-IN")}</p></div>
        ...
        {tab === "settings" && (
          <div className="space-y-3">
            ...
            {[
              ...
              { key: "support_email", label: "Support Email", placeholder: "support@bkmatka.com" },
              ...
              <div key={s.key} className="bg-card rounded-xl p-3 space-y-1 border border-border/50">
                ...
                <div className="flex gap-2">
                  <input value={settings[s.key] || ""} onChange={(e) => setSettings({ ...settings, [s.key]: e.target.value })} placeholder={s.placeholder} className="flex-1 bg-secondary text-foreground px-3 py-2 rounded-xl text-sm outline-none placeholder:text-muted-foreground" />
