...
import { ArrowRight, Sparkles, TrendingUp, Shield, Users, Zap, Trophy, Clock, Star } from "lucide-react";
...
const Index = () => {
  ...
  const upcomingCount = games.filter(g => getStatus(g.result_time) === "upcoming").length;
  ...
  return (
    <div className="min-h-screen bg-background bg-grid pb-24 relative">
          ...
          <div className="relative z-10">
            <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
              <span className="inline-flex items-center gap-1.5 bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-semibold mb-3">
                <Sparkles className="w-3 h-3" /> {appName} में स्वागत है
              </span>
              <h1 className="font-display text-2xl font-bold text-foreground mb-1">
                समझदारी से खेलो, <span className="text-gradient-gold">बड़ा जीतो</span>
              </h1>
              <p className="text-muted-foreground text-sm mb-4">India का सबसे भरोसेमंद Platform</p>
              <div className="flex gap-2">
                <button onClick={() => navigate("/games")} className="gradient-primary text-primary-foreground px-5 py-2.5 rounded-xl text-sm font-semibold shadow-glow-primary hover:opacity-90 transition-opacity flex items-center gap-1.5">
                  अभी खेलें <ArrowRight className="w-4 h-4" />
                </button>
                <button onClick={() => navigate("/results")} className="bg-secondary text-secondary-foreground px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-secondary/80 transition-colors">
                  नतीजे देखें
                </button>
              </div>
            </motion.div>
      ...
      <div className="px-4 py-2 grid grid-cols-4 gap-2">
        {[
          { icon: Zap, label: "Live", value: String(liveCount), bg: "bg-destructive/10", text: "text-destructive" },
          { icon: Clock, label: "Upcoming", value: String(upcomingCount), bg: "bg-primary/10", text: "text-primary" },
          { icon: Shield, label: "Verified", value: "100%", bg: "bg-game-blue/10", text: "text-game-blue" },
          { icon: Trophy, label: "Payout", value: "Fast", bg: "bg-game-green/10", text: "text-game-green" },
        ].map((s) => (
          <div key={s.label} className={`${s.bg} rounded-xl p-2.5 flex flex-col items-center gap-0.5 border border-border/30`}>
            <s.icon className={`w-4 h-4 ${s.text}`} />
            <p className={`font-display font-bold text-xs ${s.text}`}>{s.value}</p>
      ...
      <div className="px-4 py-3">
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={() => navigate("/games")} className="text-xs text-primary font-medium flex items-center gap-1 hover:underline">
